...
void writeCommand(unsigned char c) {
    // DC (Data Command) Pin set to Pin 62
    // OLEDCS (CS) Pin set to Pin 61
    unsigned long ulDummy;

    MAP_SPICSEnable(GSPI_BASE);
    GPIOPinWrite(GPIOA2_BASE, OLED_CS_PIN, 0); // Set the CS pin to low (Select the device)

    GPIOPinWrite(GPIOA2_BASE, OLED_DC_PIN, 0); // Set the DC pin to low to send the command byte (not data byte!)

    MAP_SPIDataPut(GSPI_BASE, c); // Send via SPI
    MAP_SPIDataGet(GSPI_BASE, &ulDummy); // Clear RX register

    MAP_SPICSDisable(GSPI_BASE);
    GPIOPinWrite(GPIOA2_BASE, OLED_DC_PIN, OLED_DC_PIN);
    GPIOPinWrite(GPIOA2_BASE, OLED_CS_PIN, OLED_CS_PIN);
}
//*****************************************************************************
...
void writeData(unsigned char c) {
    ...
    MAP_SPICSEnable(GSPI_BASE);
    GPIOPinWrite(GPIOA2_BASE, OLED_CS_PIN, 0); // Set the CS pin to low (Select OLED device, just practice in case there are multiple devices)
    GPIOPinWrite(GPIOA2_BASE, OLED_DC_PIN, OLED_DC_PIN); // Set the DC pin to high to send the data byte
